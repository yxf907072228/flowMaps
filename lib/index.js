...
var FlowMap = function (_React$Component) {
    ...
    _createClass(FlowMap, [{
        ...
        value: function test() {
            ...
            var node1 = this.addNode('zuzhi', [59, 500]);
            var node2 = this.addNode('zzuzhi', [499, 200]);
            var node3 = this.addNode('zzuzhi', [199, 100]);

            var arrow1 = this.addArrow();
            var arrow2 = this.addArrow();
        ...
        value: function _initEvents() {
            ...
            document.body.addEventListener("mousemove", function (e) {
                if (dragTarget) {
                    ...
                    if (_this3.data(dragTarget) == 'rootGroup') {
        ...
        value: function addArrow() {
            var start = arguments.length <= 0 || arguments[0] === undefined ? { x: 0, y: 0 } : arguments[0];
            var end = arguments.length <= 1 || arguments[1] === undefined ? { x: 0, y: 0 } : arguments[1];
        ...
        value: function refreshArrow(group, start, end) {
            ...
            if (start == null || end == null) {
                ...
                var xLow = nodeWidth * xLen / zLen / 2;
                var yLow = nodeHeight * yLen / zLen / 2;
        ...
        value: function addContainer(type, position) {
            ...
            var containerData = this.data(containerGroup, {
                type: 'container',
                containerType: type,
        ...
        key: 'deserialization',
        value: function deserialization(data) {
            var _this7 = this;
            ...
            this.resetPage({}, function () {
                for (var key in containers) {
                    ...
                    var containerData = _this7.data(container['id']);
                    _this7.addContainer(containerData['containerType'], container.position, Object.assign(container, containerData));
                }

                for (var _key3 in nodes) {
                    var node = nodes[_key3];
                    var nodeData = _this7.data(node['id']);
                    var _container = _this7.containers[nodeData['pid']];
                    _this7.addNode(nodeData['nodeType'], node['position'], Object.assign(node, nodeData), _container && _container.childAt(1));
                }
                ...
                for (var _key5 in arrows) {
                    ...
                    var arrowData = _this7.data(arrow['id']);
